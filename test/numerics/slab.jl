...
@testset "slab experiments" begin
    ...
    @test isapprox(sol1.state.ux, fill(slab1.ub, sol1.domain.nx, sol1.domain.ny), rtol = 0.001)

    # Case 2 #
    slab2 = TestSlab(H = 500.0, mu = 4e5, beta = 30.0, alpha = 1e-3)
    sol2 = solve_slab(slab2, 5e3, dtau_scaling = 10.0)
    @test isapprox(sol2.state.ux, fill(slab2.ub, sol2.domain.nx, sol2.domain.ny), rtol = 0.001)
end