...
@testset "deviatoric_stress! — depth-averaged (nz = 1)" begin
    ...
    @test ndims(mech.strainrate.xx) == 3   # tensor fields are always 3D
    @test size(mech.strainrate.xx, 3) == 1 # one layer for depth-averaged dynamics
    ...
    η = 2.0
    fill!(mat.eta_ice, η)
    fill!(mech.strainrate.xx,  3.0)
    fill!(mech.strainrate.yy, -1.0)
    fill!(mech.strainrate.xy,  0.5)

    deviatoric_stress!(mech, mat)

    # τ_ij = 2 η ε̇_ij
    @test all(mech.stress.xx .≈ 2 * η *  3.0)   # 12
    @test all(mech.stress.yy .≈ 2 * η * -1.0)   # -4
    @test all(mech.stress.xy .≈ 2 * η *  0.5)   # 2

    # τ_zz reconstructed from the traceless identity: -(τxx + τyy) = -(12 - 4) = -8.
    @test all(mech.stress.zz .≈ -(12.0 - 4.0))  # -8
    # Vertical-shear stresses vanish in the depth-averaged case.
    @test all(mech.stress.xz .== 0.0)
    @test all(mech.stress.yz .== 0.0)

    # Effective stress (unified formula): √(½(144 + 16 + 64) + 4) = √(112 + 4) = √116.
    # Equivalently √(τxx² + τyy² + τxx τyy + τxy²) with the reconstructed τzz.
    @test all(mech.stress.effective .≈ sqrt(116.0))
end
...
@testset "deviatoric_stress! — full-column (nz > 1)" begin
    ...
    @test size(mech.strainrate.xx, 3) == 4
    ...
    fill!(mech.strainrate.xx,  3.0)
    fill!(mech.strainrate.yy, -1.0)
    fill!(mech.strainrate.zz, -2.0)
    fill!(mech.strainrate.xy,  0.5)
    fill!(mech.strainrate.xz,  1.0)
    fill!(mech.strainrate.yz, -0.5)

    deviatoric_stress!(mech, mat)

    @test all(mech.stress.xx .≈ 2 * η *  3.0)   # 12
    @test all(mech.stress.yy .≈ 2 * η * -1.0)   # -4
    @test all(mech.stress.zz .≈ -(12.0 - 4.0))  # -8, reconstructed = 2 η ε̇_zz here
    @test all(mech.stress.xy .≈ 2 * η *  0.5)   # 2
    @test all(mech.stress.xz .≈ 2 * η *  1.0)   # 4
    @test all(mech.stress.yz .≈ 2 * η * -0.5)   # -2

    # Effective stress: √(½(144 + 16 + 64) + 4 + 16 + 4) = √(112 + 24) = √136
    @test all(mech.stress.effective .≈ sqrt(136.0))
end
